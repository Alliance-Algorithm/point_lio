...
PointCloudXYZI::Ptr feats_undistort_world(new PointCloudXYZI());     // 未经过体素滤波的世界系点云
...
void publish_frame_world_undistort(
    ...
    if (scan_pub_en) {
        // 将 feats_undistort (未经过体素滤波的body系点云) 转换到世界坐标系
        ...
        if (sync_packages(Measures)) {
            ...
            if (flg_first_scan) {
                ...
                if (imu_en) {
                    ...
                    {
                        while (Measures.lidar_beg_time > get_time_sec(
            ...
            if (!init_map) {
                ...
                for (int i = 0; i < feats_undistort->size(); i++) {
                    {
                        pointBodyToWorld(
                            &(feats_undistort->points[i]), &(feats_down_world->points[i]));
                    }
                }
                                ...
                                {
                                    {
                                        while (get_time_sec(imu_next.header.stamp)
                                               < Measures.lidar_beg_time + lidar_time_inte) {
                                            // meas.imu.emplace_back(imu_deque.front()); should add
                                            // to initialization
                                            imu_deque.pop_front();
                                            if (imu_deque.empty())
                                                break;
            ...
            if (runtime_pos_log) {
                ...
                printf(
                    ...
                    "solve: "
                    "%0.6f  ave ICP: %0.6f  map incre: %0.6f ave total: %0.6f icp: %0.6f "
                    "propogate: %0.6f \n",

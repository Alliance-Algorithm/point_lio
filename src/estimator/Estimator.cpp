...
PointCloudXYZI::Ptr cloud_body_link(new PointCloudXYZI());
PointCloudXYZI::Ptr cloud_world_link(new PointCloudXYZI());
...
void h_model_input(state_input& s, esekfom::dyn_share_modified<double>& ekfom_data)
{
    ...
    for (int j = 0; j < time_seq[k]; j++) {
...

        {
            ...
            ikd_tree.Nearest_Search(point_world_j, NUM_MATCH_POINTS, points_near, pointSearchSqDis, 2.236); // 1.0); //, 3.0); // 2.236;
    ...
    for (int j = 0; j < time_seq[k]; j++) {
        if (point_selected_surf[idx + j + 1]) {
            ...
            ekfom_data.z(m) = -norm_vec(0) * cloud_world_link->points[idx + j + 1].x - norm_vec(1) * cloud_world_link->points[idx + j + 1].y - norm_vec(2) * cloud_world_link->points[idx + j + 1].z - normvec->points[j].intensity;
            m++;
        }
    }
    effect_feat_num += effect_num_k;
}
...
void h_model_output(state_output& s, esekfom::dyn_share_modified<double>& ekfom_data)
{
    ...
    for (int j = 0; j < time_seq[k]; j++) {
        ...
        p_world << point_world_j.x, point_world_j.y, point_world_j.z;
        {
            ...
            ikd_tree.Nearest_Search(point_world_j, NUM_MATCH_POINTS, points_near, pointSearchSqDis, 2.236);

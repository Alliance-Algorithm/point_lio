...
Eigen::Matrix<double, 24, 24> process_noise_cov_input()
{
    ...
    // MTK::get_cov<process_noise_input>::type::Zero(); MTK::setDiagonal<process_noise_input, vect3,
    // 0>(cov, &process_noise_input::ng, gyr_cov_input);// 0.03
    // MTK::setDiagonal<process_noise_input, vect3, 3>(cov, &process_noise_input::na,
    // acc_cov_input); // *dt 0.01 0.01 * dt * dt 0.05 MTK::setDiagonal<process_noise_input, vect3,
    // 6>(cov, &process_noise_input::nbg, b_gyr_cov); // *dt 0.00001 0.00001 * dt *dt 0.3 //0.001
    // 0.0001 0.01 MTK::setDiagonal<process_noise_input, vect3, 9>(cov, &process_noise_input::nba,
    // b_acc_cov);   //0.001 0.05 0.0001/out 0.01
    return cov;
}
...
Eigen::Matrix<double, 30, 30> process_noise_cov_output()
{
    ...
    // MTK::get_cov<process_noise_output>::type::Zero(); MTK::setDiagonal<process_noise_output,
    // vect3, 0>(cov, &process_noise_output::vel, vel_cov);// 0.03
    // MTK::setDiagonal<process_noise_output, vect3, 3>(cov, &process_noise_output::ng,
    // gyr_cov_output); // *dt 0.01 0.01 * dt * dt 0.05 MTK::setDiagonal<process_noise_output,
    // vect3, 6>(cov, &process_noise_output::na, acc_cov_output); // *dt 0.00001 0.00001 * dt *dt
    // 0.3 //0.001 0.0001 0.01 MTK::setDiagonal<process_noise_output, vect3, 9>(cov,
    // &process_noise_output::nbg, b_gyr_cov);   //0.001 0.05 0.0001/out 0.01
    // MTK::setDiagonal<process_noise_output, vect3, 12>(cov, &process_noise_output::nba,
    // b_acc_cov);   //0.001 0.05 0.0001/out 0.01
    return cov;
}
...
Eigen::Matrix<double, 24, 24> df_dx_input(state_input& s, const input_ikfom& in)
{
    ...
    // Eigen::Matrix<state_ikfom::scalar, 2, 1> vec = Eigen::Matrix<state_ikfom::scalar, 2,
    // 1>::Zero(); Eigen::Matrix<state_ikfom::scalar, 3, 2> grav_matrix; s.S2_Mx(grav_matrix, vec,
    // 21);
    ...
    for (int j = 0; j < time_seq[k]; j++) {
...

        {
            ...
            ikdtree.Nearest_Search(
                point_world_j, NUM_MATCH_POINTS, points_near, pointSearchSqDis,
                2.236); // 1.0); //, 3.0); // 2.236;
...
void h_model_output(state_output& s, esekfom::dyn_share_modified<double>& ekfom_data)
{
    ...
    for (int j = 0; j < time_seq[k]; j++) {
        ...
        p_world << point_world_j.x, point_world_j.y, point_world_j.z;
        {
            ...
            ikdtree.Nearest_Search(
                point_world_j, NUM_MATCH_POINTS, points_near, pointSearchSqDis, 2.236);
